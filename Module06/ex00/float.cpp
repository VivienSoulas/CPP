...
bool Float::ft_parse( std::string input )
{
	if (input.empty())
		return (false);
...
void Float::convert( std::string input )
{
	...
	else
	{
		...
		std::cout << "Float: " << n_prev << "." << n_aft << "f\n";
	}
}

int Float::ft_atoi( std::string input, int *overflow, size_t dot, int flag )
{
	long long n = 0;
	int sign = 1;
	size_t i = 0;
